...
struct Trivia
{
  ...
  bool ask_question (int num_ask)
  {
	...
	return false;
      ...
      {
	cout << "Warning - there (is/are) only " << size <<
	  " trivia question(s) in the list." << endl;
	return false;
      }
    else
      {
	...
	while (itr.has_next () && count < num_ask)
	  {
	    ...
	    if (user_answer.compare (current.answer) == 0)
	      {
		cout << "Your answer is correct. You receive " << current.
		  value << " points." << endl;
		score += current.value;
	      }
	    else
  ...
  void initial_questions (int version)
  {
    ...
    score = 0;
    if (version > -1)
      {				// Version 0 selects this list only
	add
	  ("How long was the shortest war on record? (Hint: how many minutes)",
	   "38", 100);
      }
    if (version > 0)
      {				// Version 1 adds this block and the previous
	add
